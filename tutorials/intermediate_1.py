...
print(f'{my_dataframe[['Wage','Value']].mean()} and main mean is {my_dataframe[['Wage','Value']].mean().mean()}')
my_dataframe['Under_paid'] =  (my_dataframe['Wage'].values <= 200000) & (my_dataframe['Value'].values >= 100000000)

# Getting the players who are under paid and have high value
print(f'Under Paid Players {my_dataframe[my_dataframe['Under_paid']]}')
underpaid= my_dataframe[my_dataframe['Under_paid']]
print(underpaid['Wage'])
...
plt.scatter(my_dataframe['Value'],my_dataframe['Wage'], alpha=0.7, color='blue' , label =' All players')
plt.scatter(underpaid['Value'],underpaid['Wage'],color = 'red', alpha = 0.7, label='Under paid player')
plt.plot(my_dataframe['Wage'],y_ped)
